...
<li class="selected"><a id="tab1" href="javascript:void(1)"><em>{sugar_translate label='LBL_ADDRESS_INFORMATION' module='AOS_Quotes'}</em></a></li>
...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select name="{$fields.curier_c.name}" 
id="{$fields.curier_c.name}" 
title=''       
>
{if isset($fields.curier_c.value) && $fields.curier_c.value != ''}
{html_options options=$fields.curier_c.options selected=$fields.curier_c.value}
{else}
{html_options options=$fields.curier_c.options selected=$fields.curier_c.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.curier_c.options }
{capture name="field_val"}{$fields.curier_c.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.curier_c.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.curier_c.name}" 
id="{$fields.curier_c.name}" 
title=''          
>
{if isset($fields.curier_c.value) && $fields.curier_c.value != ''}
...
<input
id="{$fields.curier_c.name}-input"
name="{$fields.curier_c.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.curier_c.name}-image"></button><button type="button"
id="btn-clear-{$fields.curier_c.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.curier_c.name}-input', '{$fields.curier_c.name}');sync_{$fields.curier_c.name}()"><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
</span>
{literal}
<script>
SUGAR.AutoComplete.{/literal}{$ac_key}{literal} = [];
...
{literal}
(function (){
var selectElem = document.getElementById("{/literal}{$fields.curier_c.name}{literal}");
if (typeof select_defaults =="undefined")
select_defaults = [];
select_defaults[selectElem.id] = {key:selectElem.value,text:''};
//get default
...
YUI().use("autocomplete", "autocomplete-filters", "autocomplete-highlighters", "node","node-event-simulate", function (Y) {
{/literal}
SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.curier_c.name}-input');
SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.curier_c.name}-image');
SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.curier_c.name}');
{literal}
function SyncToHidden(selectme){
var selectElem = document.getElementById("{/literal}{$fields.curier_c.name}{literal}");
var doSimulateChange = false;
...
//global variable 
sync_{/literal}{$fields.curier_c.name}{literal} = function(){
SyncToHidden();
}
function syncFromHiddenToWidget(){
var selectElem = document.getElementById("{/literal}{$fields.curier_c.name}{literal}");
...
SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.simulate('keyup');
for (i=0;i<selectElem.options.length;i++){
if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.curier_c.name}-input{literal}'))
SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
}
}
YAHOO.util.Event.onAvailable("{/literal}{$fields.curier_c.name}{literal}", syncFromHiddenToWidget);
{/literal}
SUGAR.AutoComplete.{$ac_key}.minQLen = 0;
SUGAR.AutoComplete.{$ac_key}.queryDelay = 0;
SUGAR.AutoComplete.{$ac_key}.numOptions = {$field_options|@count};
...
{/if}  
<input type='text' name='{$fields.curier_account_c.name}' 
id='{$fields.curier_account_c.name}' size='30' 
maxlength='255' 
value='{$value}' title=''      >
</tr>
{/capture}
...
{if strlen($fields.handling_c.value) <= 0}
{assign var="value" value=$fields.handling_c.default_value }
{else}
{assign var="value" value=$fields.handling_c.value }
{/if}  
<input type='text' name='{$fields.handling_c.name}' 
id='{$fields.handling_c.name}' size='30' maxlength='26' value='{sugar_number_format var=$value}' title='' tabindex='0'
>
...
{capture name="label" assign="label"}{sugar_translate label='LBL_TRANSPORTATION' module='AOS_Quotes'}{/capture}
{$label|strip_semicolon}:
</td>
{counter name="fieldsUsed"}
...
{if strlen($fields.transportation_c.value) <= 0}
{assign var="value" value=$fields.transportation_c.default_value }
{else}
{assign var="value" value=$fields.transportation_c.value }
{/if}  
<input type='text' name='{$fields.transportation_c.name}' 
id='{$fields.transportation_c.name}' size='30' maxlength='26' value='{sugar_number_format var=$value}' title='' tabindex='0'
>
<td valign="top" id='_label' width='12.5%' scope="col">
&nbsp;
</td>
...
addToValidate('EditView', 'curier_account_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_CURIER_ACCOUNT' module='AOS_Quotes' for_js=true}{literal}' );
addToValidate('EditView', 'curier_c', 'enum', false,'{/literal}{sugar_translate label='LBL_CURIER' module='AOS_Quotes' for_js=true}{literal}' );
addToValidate('EditView', 'handling_c', 'currency', false,'{/literal}{sugar_translate label='LBL_HANDLING' module='AOS_Quotes' for_js=true}{literal}' );
addToValidate('EditView', 'transportation_c', 'currency', false,'{/literal}{sugar_translate label='LBL_TRANSPORTATION' module='AOS_Quotes' for_js=true}{literal}' );
addToValidate('EditView', 'order_id_c', 'int', false,'{/literal}{sugar_translate label='LBL_ORDER_ID' module='AOS_Quotes' for_js=true}{literal}' );
addToValidate('EditView', 'user_profile_id_c', 'int', false,'{/literal}{sugar_translate label='LBL_USER_PROFILE_ID' module='AOS_Quotes' for_js=true}{literal}' );
